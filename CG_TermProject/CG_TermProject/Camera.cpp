...
void camera() {
	...
	//CPos = VAngleX_Rot * VAngleY_Rot * glm::vec4(CPos, 1.f);

	//glm::vec3 CDir(1.f + carAddX, -0.5f + carAddY, 0.0f + carAddZ);
	//CDir = VAngleX_Rot * VAngleY_Rot * glm::vec4(CDir, 1.f);

	//glm::vec3 cameraPos;      //--- 카메라 위치 (어디서 볼건지)
	//glm::vec3 cameraDirection;               //--- 카메라 바라보는 방향 (어디볼건지 하면될듯)
	//glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);                     //--- 카메라 위쪽 방향->벡터임(방향만) (음수하면 화면 상하거꾸로보임)

	//cameraPos = glm::vec3(CPos.x, CPos.y, CPos.z);      //--- 카메라 위치 (어디서 볼건지)
	//cameraDirection = glm::vec3(CDir.x, CDir.y, CDir.z);               //--- 카메라 바라보는 방향 (어디볼건지 하면될듯)
